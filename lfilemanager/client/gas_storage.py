...
class GASDriveStorage(Storage):
    ...
    def _save(self, name, content):
        ...
        if response.status_code in [200, 301, 302]:
            try:
                ...
                file_id = data.get("id") or data.get("fileId") or data.get("id_archivo") or data.get("file_id")
                
                if file_id:

                    return str(file_id)
                else:
                    raise Exception(f"El JSON de GAS no trajo ninguna llave conocida de ID. Respuesta recibida: {data}")
                # =================================================================
    ...
    def exists(self, name):
        # Siempre devolvemos False para que Django no intente 
